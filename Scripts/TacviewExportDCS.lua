...
{
	-- Log parameters
	EnableDetailedProfiling=false,		-- Set to true to enable full profiling data dump
	ProfilingPeriod=60,					-- (seconds) delay between profiling data dump
	FrameIndex=0,						-- Used as a profiling reference
...
--		[14]				=	"",
	...
	FastFile=
	{
		...
		Write=function(self,Data)
			...
			if self.CacheSize+dataLength>=self.MaxCacheSize then
...
--				log.info(string.format('[Tacview.FastFile.Write] Flushing data cache to disk FrameIndex,CacheSize,len(Data)=[%05u %06u %06u]',Tacview.FrameIndex,self.CacheSize,dataLength));
			...
			self.CacheSize=self.CacheSize+dataLength;
	...
	-- Profile Tools
	BeginProfile=function(self,name)

		if self.ProfileData[name]==nil then

			self.ProfileData[name]={};
			self.ProfileData[name].TotalTime=0;
			self.ProfileData[name].CallCount=0;

		end

		self.ProfileData[name].BeginTime=os.clock();

	end,

	EndProfile=function(self,name)

		local ElapsedTime=os.clock()-self.ProfileData[name].BeginTime;

		self.ProfileData[name].TotalTime=self.ProfileData[name].TotalTime+ElapsedTime;
		self.ProfileData[name].CallCount=self.ProfileData[name].CallCount+1;
	...
	BeginLog=function(self)
		...
		self.ProfileLastUpdateTime=0;		-- Used to profile this export script update
		self.ProfileFrameCount=0;
		self.ProfileTotalDCSFrameTime=0;
		self.ProfileTotalTacviewFrameTime=0;

		self.ProfileData={};				-- Used to store detailed profiling data
		...
		log.info("[Tacview.BeginLog] Starting Tacview 1.5.1 export script");
		...
		for _,TargetFolder in pairs(TargetFolders) do
			...
			if self.AcmiFile:Create(TargetFolder..FileName)==true then
				log.info("[Tacview.BeginLog] Exporting data to ["..TargetFolder..FileName.."]");
		...
		self.AcmiFile:Write("Recorder=Tacview 1.5.1\n");
		...
		if CameraPosition then
			...
			CameraCoordinates=LoLoCoordinatesToGeoCoordinates(CameraTranslation.x,CameraTranslation.z)
		else
			CameraCoordinates={}
			CameraCoordinates.longitude=41
			CameraCoordinates.latitude=43
		end
	...
	UpdateLog=function(self)
		...
		-- Profiling
		local UpdateBeginTime=os.clock();
		...
		self:BeginProfile('LoGetWorldObjects(units) [C++]');	-- PROFILE
		local DefaultObjectsList=LoGetWorldObjects();			-- LoGetWorldObjects("units");
		self:EndProfile('LoGetWorldObjects(units) [C++]');		-- PROFILE
		...
		self:BeginProfile('AddUpdateObjects(units)');			-- PROFILE
		if self:AddUpdateObjects(DefaultObjectsList,FormatedTime,ShouldUpdateLog)==true then
			FormatedTime=nil;
		end
		self:EndProfile('AddUpdateObjects(units)');				-- PROFILE
		...
		self:BeginProfile('LoGetWorldObjects(ballistic) [C++]');-- PROFILE
		local BallisticObjectsList=LoGetWorldObjects("ballistic");
		self:EndProfile('LoGetWorldObjects(ballistic) [C++]');	-- PROFILE

		self:BeginProfile('AddUpdateObjects(ballistic)');		-- PROFILE
		if self:AddUpdateObjects(BallisticObjectsList,FormatedTime,ShouldUpdateLog)==true then
			FormatedTime=nil;
		end
		self:EndProfile('AddUpdateObjects(ballistic)');			-- PROFILE

		-- Remove destroyed objects
		self:BeginProfile('RemoveObjects');						-- PROFILE
		self:RemoveObjects(DefaultObjectsList,BallisticObjectsList,FormatedTime);
		self:EndProfile('RemoveObjects');						-- PROFILE

		-- Profiling
		local UpdateEndTime=os.clock();

		if self.ProfileLastUpdateTime>0 then

			local DCSFrameTime=UpdateBeginTime-self.ProfileLastUpdateTime;
			local TacviewFrameTime=UpdateEndTime-UpdateBeginTime;
			local FrameTime=DCSFrameTime+TacviewFrameTime;

			self.FrameIndex=self.FrameIndex+1;

			-- Warning When Too Much CPU

			if FrameTime>0 then
--				TacviewRatio=TacviewFrameTime/FrameTime;
--				log.info(string.format('[Tacview.UpdateLog] Tacview frame profile FrameIndex,FrameTime,Ratio=[%05u %06.3f %04.1f%%] of the frame',self.FrameIndex,FrameTime,TacviewRatio*100));
			end

			-- Update Stats

			self.ProfileTotalDCSFrameTime=self.ProfileTotalDCSFrameTime+DCSFrameTime;
			self.ProfileTotalTacviewFrameTime=self.ProfileTotalTacviewFrameTime+TacviewFrameTime;
			self.ProfileFrameCount=self.ProfileFrameCount+1;

			-- Dump Stats Time to Time to not Spam the Log
			local TotalFrameTime=self.ProfileTotalDCSFrameTime+self.ProfileTotalTacviewFrameTime;

			if TotalFrameTime>=self.ProfilingPeriod then

				-- Calculate Performance

				local FrameRate;
				local DCSFrameRatio;
				local TacviewFrameRatio;

				if TotalFrameTime>0 then
					FrameRate=self.ProfileFrameCount/TotalFrameTime;
					DCSFrameRatio=self.ProfileTotalDCSFrameTime/TotalFrameTime;
					TacviewFrameRatio=self.ProfileTotalTacviewFrameTime/TotalFrameTime;
				else
					FrameRate=0;
					DCSFrameRatio=0;
					TacviewFrameRatio=0;
				...
				if self.ProfileTotalDCSFrameTime>0 then
					DCSFrameRate=self.ProfileFrameCount/self.ProfileTotalDCSFrameTime;
				else
					DCSFrameRate=0;
				end
				...
				log.info(string.format('[Tacview.UpdateLog] Profile Time=[%.1fs] Frame Rate=[%.1f fps] w/o Export=[%.1f fps] DCS Ratio=[%.1f%%] Export Ratio=[%.1f%%]',TotalFrameTime,FrameRate,DCSFrameRate,DCSFrameRatio*100,TacviewFrameRatio*100));

				-- Display Detailed Profile

				if self.EnableDetailedProfiling==true then

					log.info(string.format('-------- Profiled Section -------- | CallCount | Ratio | Total |'));

					-- Sort by Name
					SectionList={}
					for n in pairs(self.ProfileData) do table.insert(SectionList,n) end
					table.sort(SectionList)

					-- Dump Info
					for Index,SectionId in ipairs(SectionList) do

						local Section=self.ProfileData[SectionId];

						local TimeRatio;
						local TotalTimeRatio;

						if self.ProfileTotalTacviewFrameTime>0 then
							TimeRatio=Section.TotalTime/self.ProfileTotalTacviewFrameTime;
							TotalTimeRatio=Section.TotalTime/TotalFrameTime;
						else
							TimeRatio=0;
							TotalTimeRatio=0;
						end

						log.info(string.format('%-35s| %9d |%5.1f%% |%5.1f%% |',SectionId,Section.CallCount,TimeRatio*100,TotalTimeRatio*100));
					end
				end
		...
		self.ProfileLastUpdateTime=UpdateEndTime;
	end,
		...
		for ID,Object in pairs(CurrentObjectsList) do
			...
			if ObjectType~=-1 then	-- Ignore some objects (like particles clouds)
				...
				if not self.LastObjectsStatus[ID] then
					...
					if string.sub(PilotName,1,7)=="Pilot #" or string.sub(PilotName,1,6)=="Unit #" then
						PilotName="";		-- trim useless names to reduce clutter
					end
	...
	UpdateObject=function(self,ID,CurrentObjectData,ObjectType,PrevObjectData,FormatedTime,ShouldUpdateLog)
		...
		if self.IsSimpleObject(ObjectType)==false and ObjectType~=0x46 and ObjectType<0x80 then
			...
			if not PrevObjectData or CurrentObjectData.Position.z~=PrevObjectData.Position.z or CurrentObjectData.Position.x~=PrevObjectData.Position.x or CurrentObjectData.Heading~=PrevObjectData.Heading then
				AdvancedLog=AdvancedLog..string.format(",,%.2f,%.2f,%.1f",CurrentObjectData.Position.z,CurrentObjectData.Position.x,math.mod(math.deg(CurrentObjectData.Heading),360));
			...
			if PlayerPlaneId==ID then
				...
				if MainPanel~=nil and type(MainPanel)=="table" then
					MainPanel:update_arguments();		-- Ensure cockpit info is up-to-date even when not in cockpit (works in DCS only)
				else
					MainPanel=nil						-- Make sure non table panels are used (the A-10C returns a number when not available)
				end

				-- Detect Plane (DCS) and corresponding parameters
				-- C:\Program Files\Eagle Dynamics\DCS World\Mods\aircraft\F-5E\Cockpit\Scripts\mainpanel_init.lua
...
--AdvancedLogChangeDetected=true;
					...
					elseif PlayerPlaneName=="f-5e-3" then

						IASIndex =8;
						IASInput ={0.0,   80.0, 100.0, 170.0,  190.0, 200.0, 230.0,  250.0, 290.0, 300.0,  350.0, 390.0, 400.0, 500.0, 550.0, 650.0, 700.0, 750.0, 800.0, 850.0};
						IASOutput={0.0, 0.0435,   0.1, 0.318, 0.3745, 0.397, 0.4495, 0.482,  0.54, 0.553, 0.6145, 0.658, 0.668, 0.761, 0.801, 0.877, 0.909, 0.942, 0.972,   1.0};
						IASFactor=0.514444;				-- kts to m/s factor

						LeftThrottleIndex=104;
						RightThrottleIndex=105;
						ThrottleFactor=1;
...
--						AirBrakesIndex=124;
				...
				-- It looks like that IAS is now always exported in m/s since DCS World 1.5
--[[
...
--]]
	...
	EndLog=function(self)
		...
		if self.AcmiFile then
			...
			log.info("[Tacview.EndLog] Telemetry file closed");
		end
