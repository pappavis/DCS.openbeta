...
function _M.sendUpdate()
	...
	if type(mainPanel) ~= "number" then
		for k, arg_num in pairs(_M.exportedCockpitArguments) do
			local arg = round2(GetDevice(0):get_argument_value(arg_num), 2)
			_M.KVS.set(k, arg)
		end
	end

	for k, arg_num in pairs(_M.exportedDrawArguments) do
		local arg = tostring(round2(LoGetAircraftDrawArgumentValue(arg_num), 2))
		_M.KVS.set(k, arg)
	end
...
function _M.processMessage(msg)
	...
	if msg.action and msg.action == "subscribe" then
		for _, key in pairs(msg.keys) do
			...
			if key:match("^e.*") then
				local exportDev = GetDevice(705)
				if type(exportDev) ~= "number" then
					...
					_M.exportedDrawArguments[key] = tonumber(key:sub(2))
					_M.KVS.data[key] = nil -- ensure it is sent in the next update even if already subscribed
					_M.KVS.dirty[key] = true
				end
			end
