...
gExportInterval = 0.333        -- frequency of export events (sec)
gTickCount = 0

function LuaExportStart()

  package.path  = package.path..";.\\LuaSocket\\?.lua"
  package.cpath = package.cpath.."C:\Program Files (x86)\Lua\5.1\clibs\socket\?.dll"
...
assert(udp:send(string.format("%s\n", "*")))
assert(udp:send(string.format("%d;%f;%f;%f;%d;%d;%d;\n", 0, myInfo.Position.x/1000, myInfo.Heading*57.3, myInfo.Position.z/1000, myInfo.Type.level2, myInfo.Type.level3, myInfo.Type.level4)))


local o = LoGetWorldObjects()
for k,v in pairs(o) do
	if v.CoalitionID==1 and v.Type.level1<=3 and (v.Type.level2==1 or v.Type.level2==2 or v.Type.level2==8 or v.Type.level2==9 or v.Type.level2==16 or v.Type.level2==17) then
	assert(udp:send(string.format("%d;%f;%f;%f;%d;%d;%d;\n", 1, v.Position.x/1000, v.Heading*57.3, v.Position.z/1000, v.Type.level2, v.Type.level3, v.Type.level4)))
	end
	if v.CoalitionID==2 and v.Type.level1<=3 and (v.Type.level2==1 or v.Type.level2==2 or v.Type.level2==8 or v.Type.level2==9 or v.Type.level2==16 or v.Type.level2==17)   then
	assert(udp:send(string.format("%d;%f;%f;%f;%d;%d;%d;\n", 2, v.Position.x/1000, v.Heading*57.3, v.Position.z/1000, v.Type.level2, v.Type.level3, v.Type.level4)))
	end
end
